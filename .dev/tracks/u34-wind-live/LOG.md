...
- 2026-09-25 rebase op ff17c21 (U41): conflicten isoline-layer (u_weight_b + lineOpacity) en scrubber (trackElement/shownOffset + timeline-void); sticky daglabels alleen buiten de Web-Animation-slide (tijdens afspelen labels in de baan, geen Layerize per tik). Zoekveld begint leeg bij openen (ster blijft geldig, sluiten herstelt). typecheck 0, pnpm test 0 (48/321). PO: U41-crossfade van isolijnen "ziet er slecht uit" → gemeld aan orkestrator, niet door U34 aangeraakt.
## 2026-09-25 — U41 ingreep 1 terug (besluit orkestrator/PO: het beeld wint)
- IsolineLayer: traceTimes = altijd de exacte getweende tijd, selectShown = exacte snede of laatst binnengekomen, nooit overvloeiing; ISOLINE_TRACE_HZ-grens op nieuwe verzoeken tijdens afspelen (PO: "terug naar vóór deze optimalisatie" → 60, in de praktijk geen grens; de latest-wins-tracer begrenst op de worker). Rest van U41 ongemoeid.
- Stadstemperaturen: geïnterpoleerd in stappen van 10 min tijdlijntijd (PO: minder belangrijk).
- Isobaren (MIP-14): ISOBAR_BLUR 18 passes (σ ≈ 3,5 cel); temporeel elk uurframe gewogen met buren [1,2,3,2,1] vóór de B-spline (isolineFrameWeights, zowel lijnen als label-worker); lijnen 0,27 (⅓ van 0,8); adaptieve stap alleen op nieuwe uurstap + moveend.
- Meting (U41-script, prod-preview 4341 op live :8080, swiftshader, 30 s afspelen temperatuurmodus): `SCENARIOS=temperatuur direnv exec /home/mthq/motregen node ../.dev/tracks/u41-stil-in-rust/measure.mjs http://localhost:4341/ u34-continu` → traces 107 (U41: 5, vóór U41: 114), worker-CPU 5,84 s (U41 5,15; vóór 1,81), hoofddraad-CPU 2,55 s (U41 8,54; vóór 1,99), taak 3,35 s, regen 2 draws/s (U41 19,9; vóór 1,9), isolijn 3,6/s, gpu-CPU 312 s. Lezing: terug op het vóór-U41-niveau; op deze host zonder GPU verhongert de frame-loop weer door vector+vulpass per snede op swiftshader — op echte hardware goedkoop.
- Receipts: typecheck 0; `pnpm test` 0 (48, 321); build 0. Geen e2e.
- 2026-09-25 PO live: 'Nu' als pil in de urenbalk (uurlabels binnen 24 px wijken); daglabels ook tijdens afspelen sticky via compositor-keyframes (stickyKeyframes: knikken van min(max(a+s, inset), b+s) als tussenstops, zelfde startTime als de baan). typecheck 0, pnpm test 0 (48/322).
- 2026-09-25 PO live: gestreepte randen weg; baan loopt tegen een muur aan begin/eind (wallClamp, C1-knie 40 px) en de cursor beweegt dan zelf (cursorX); tik rekent met de werkelijke verschuiving; compositor-slide alleen buiten de knieën en stopt vóór de rechterknie. typecheck 0, pnpm test 0 (48/323).
- 2026-09-25 PO live: afspelen blijft nooit meer hangen — interactie pauzeert, hervat na 4 s rust (RESUME_IDLE_MS); voorbij de horizon hervat = terugglijden naar het begin i.p.v. stoppen. Scrubber-windgrafiek (vlakte in Beaufort-kleuren, vlagen gestippeld, hulplijnen Bft 3/5/7/9, readout "3 ⌇ 6 Bft"); alle weergaven (regen+bewolking, wolkenlagen, wind) vloeien over op de focus-tweens (hover én pin) met opacity + 6 px schuif; wolkenlagen laden bij hover. typecheck 0, pnpm test 0.
- 2026-09-25 PO live: geen y-as in de scrubber (regen-hulplijnen en Bft-lijnen/labels weg), waarden bij de cursor (bewolkingsband %, wolkenlagen "hoog 80%" enz., wind "3 ⌇ 6 Bft"); temperatuurgrafiek (gevoel, kaartpalet, "14°", view 'temperature'); daglabels in de urenbalk (sticky blijft); gepinde kolom niet meer getint (alleen hover op niet-gepinde). typecheck 0, pnpm test 0 (48/324). cloud-section.spec: Gevoel → 'temperature'.
- 2026-09-25 PO live: screenshot (tmp/u34-*.png) toonde halve uurlabels onder daglabels → middernacht-uurlabel weg, daglabel-achtergrond loopt zacht uit; zonsopkomst/-ondergang-glyph met stralen.
- 2026-09-25 PO live: windgrafiek — vlagen als gestapelde band op de gemiddelde wind (bandRuns), cursor twee puntjes '2 Bft' (label onder) en '3 Bft' (vlaag, label boven).
- 2026-09-25 PO live: afspelen tot het einde van de tijdlijn (horizon null), 2 s stilstaan aan het eind, dan terugglijden; tempo als vroeger (timelinePlaybackRate op 8 u); cursorTemperature-TDZ-crash gefixt + test met alle app-props.
- 2026-09-25 PO live: temperatuurgrafiek met gevoel (vlakte+lijn) én luchttemperatuur (dunne lijn), cursor twee puntjes '11° gevoel' / '13°'. typecheck 0, pnpm test 0 (48/325).
- 2026-09-25 PO live: temperatuurgrafiek mooier — lijnen gekleurd met het palet (stroke-verloop), lint tussen gevoel en lucht (bandRuns, .38), vlakte naar lichte waas (.12); label '13° lucht'. typecheck 0, pnpm test 0.
- 2026-09-25 PO live (dichtheid soms lager): adjustBudget begint een nieuwe meting bij een maxFps-wissel (U41 idle 30↔60 gaf een valse ×0,78) en herstelt al onder 1,10× (was 1,03×, bleef na een dip hangen). Onzichtbare particles (loef-marge, uitdovend) tellen nog in het budget — keuze voor PO.
- 2026-09-25 PO live: wolkenlabels hoogwolken/middenwolken/laagwolken, 0% verborgen.
- 2026-09-25 PO live: scrubber altijd de drie wolkenlagen (volle hoogte) met regen eroverheen; bewolkingsband niet meer meegegeven (component ondersteunt cloudCover nog); wolkenmodus = laagwaarden bij de cursor + kaartsluier; lagen laden altijd na de initial-fase. cloud-section.spec bijgewerkt (niet gedraaid). typecheck 0, pnpm test 0 (48/325).
- 2026-09-25 PO live: wolkenlagen standaard 0,66 dekking, wolkenmodus tweent naar 1.
- 2026-09-25 PO live: wolkenlagen standaard 0,5; regenlaag op de kaart verdwijnt in de temperatuurmodus volledig (1 − focus i.p.v. FOCUS_DIM).
- 2026-09-25 PO live: tijd weer eigen kolom 'Uur' (knop), Weer-kolom apart (icoon + regen); klik op rij/tijd → scrubber springt (jumpToTime, pauze + hervat na 4 s). 'Herstart niet' (halverwege, Gevoel): niet te reproduceren (Gevoel 20 s afspelen loopt; End → terugglijden → loopt); vermoedelijk spatie op de gefocuste slider (bewuste pauze zonder zichtbare knop). typecheck 0, pnpm test 0 (48/326).
- 2026-09-25 PO live: Uur-kolom smal en kop links (spacing Uur/Weer); klok in rust 66 % + blur, bij hover/focus vol; ▶ in de klok bij bewuste pauze (spatie; onPauseToggle → userPaused), klik speelt verder. typecheck 0, pnpm test 0 (48/327).
- 2026-09-25 PO live: H/L bij de isobaren — pressureExtrema (3×3-toets, venster ±150 km volledig geldig, strikt extreem, ≥ 2 hPa prominentie) op het gladgestreken drukveld; markers alleen in beeld, dekking = windFocus; bijgewerkt met de isobaarstap (uurstap + moveend). typecheck 0, pnpm test 0.
- 2026-09-25 PO live: H/L vloeiend — per uurframe gecachete centra, tussen twee uren per soort gekoppeld (< 300 km) en geïnterpoleerd, verschijnen/verdwijnen vloeit; vanuit drawLayers met een pool markers; dekking = windFocus × fade.
- 2026-09-25 PO live: H/L-pool overschreef className (weg: maplibregl-marker → geen positionering); nu classList.toggle. Gecontroleerd (uitgezoomd, windmodus): L zichtbaar met maplibregl-marker. PO: 'nu staan ze er op'.
- 2026-09-25 PO live: H/L niet meer als MapLibre-markers maar in de windlaag (markAtlas H/L met halo, instanced quads op Mercator-positie, 30 CSS-px, na de composite); App geeft per tik positie+dekking (setPressureMarks). Gecontroleerd: L getekend binnen de 1018-lus bij Zeeland; na verlaten windmodus leeg. typecheck 0, pnpm test 0 (48/329).
