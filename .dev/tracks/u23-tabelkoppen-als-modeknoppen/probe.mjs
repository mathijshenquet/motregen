...
for (const [name, device] of Object.entries(viewports)) {
  for (const theme of ['light', 'dark']) {
    ...
    const top = () => page.evaluate(() => { const t = document.querySelector('.forecast-panel table'); if (t && innerWidth < 960) { t.style.scrollMarginTop = (innerWidth < 960 ? document.querySelector('.scrubber').getBoundingClientRect().height + 8 : 120) + 'px'; t.scrollIntoView({ block: 'start' }) } })
    await top()
