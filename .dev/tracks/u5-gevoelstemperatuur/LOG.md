...
- `pnpm e2e` herkansing (nadat U2 de poorten vrijgaf): E2E-EXIT: 0, 3 passed (1,7 m). De warme transfer is nog steeds 3× `feels_like_c`, 5.430 B.
- Status: alle gates groen. Open voor PM: (1) de band 10–15 °C is een afwijking van de letterlijke MIP-10-tekst, zie docs/fields.md. De PO beslist of die blijft en hoe breed. (2) Het bijschrift "° gevoel" en de secundaire luchttemp zijn niet visueel gescreenshot: beoordeel ze op zicht. (3) Repro: `direnv exec . cargo test --workspace`, `direnv exec . cargo clippy --workspace -- -D warnings`, en in web/: `direnv exec .. pnpm typecheck|test|build|e2e` (e2e niet gelijktijdig met een andere worktree: vaste poorten 8185/4185).
