...
describe('hourly forecast', () => {
  it('selects the next 24 whole hours and joins both fields by nearest time', () => {
    ...
    const rows = buildHourlyForecast(rain, radiation, start)

    expect(rows).toHaveLength(24)
    expect(rows[0]).toEqual({ epoch: Date.parse('2026-08-28T16:00:00Z'), rainIndex: 0, radiationIndex: 0 })
  ...
  it('leaves unavailable radiation empty without hiding rain', () => {
    ...
    expect(buildHourlyForecast(rain, [], start, 1)).toEqual([
      { epoch: Date.parse('2026-08-28T16:00:00Z'), rainIndex: 0, radiationIndex: null },
    ])
  })
})
