...
test('dev panel only behind ?dev, grouped, every control explained', async ({ page }, testInfo) => {
  ...
  await expect(groups.locator('> summary')).toHaveText(['Temperatuur', 'Diagnose'])
  // Alleen de eerste groep start open.
  await expect.poll(() => groups.evaluateAll((elements) => elements.map((element) => (element as HTMLDetailsElement).open))).toEqual([true, false])
